...
combined in a single framework to address a long-standing gap in population
genetic inference: estimating not only the topology and proportions of an
admixture graph but also the timing of each admixture event.
...
\paragraph{Edge over-detection in simple scenarios.}
In S2 ($K=1$), the greedy search inferred the correct number of admixture
edges in 45\% of seeds; in the remaining 55\%, a spurious second edge was
added to the same target population.
Notably, the precision on admixture \emph{target populations} was 100\%:
no non-admixed population was falsely labelled as admixed.
The over-detection reflects the BIC penalty being insufficiently
restrictive for seven populations, where the ratio of free parameters to
F-statistics is high enough that a spurious secondary source can improve
the fit beyond the penalty.
In S3 ($K=2$, ten populations), the richer F-statistic structure provides
stronger BIC discrimination, and the exact topology was recovered in 100\%
of seeds.
