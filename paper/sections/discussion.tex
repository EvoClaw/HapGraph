...
\paragraph{Timing estimation: resolution and scope.}
The 95\% HDI for $T$ achieved 95--97.5\% empirical coverage in the
simulation benchmarks, but with mean widths of $\sim$43 generations
spanning nearly the entire plausible range.
Inspection of individual estimates shows that $\hat{T}$ clusters around
$\sim$28 generations regardless of the true $T$ (correlation 0.52 in S2,
0.75 in S3), confirming that timing is not identifiable in these simulations.
The correct coverage reflects approximately uninformative posteriors,
not precise inference.
This limitation is expected and understood:
...
and topology accuracy would illuminate the cost of the two-stage pipeline
