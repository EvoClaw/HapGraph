...
equation~(\ref{eq:ibd_trunc}) by a sample-size-dependent correction
$\approx 2N_e / n_{\mathrm{source}}$ generations \citep{Palamara2012}.
This correction is small when whole-genome IBD data are available
(many segments reduce the correction); our 1000~Genomes analysis uses
hap-ibd on all autosomes ($\approx35$~Morgan), providing sufficient
signal.
For the simulation benchmark, which uses a single 50~Mbp chromosome
($0.5$~Morgan), timing estimates carry larger uncertainty and should be
