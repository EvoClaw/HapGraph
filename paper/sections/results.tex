...
The 95\% highest-density interval for $T$ achieved 95\% empirical coverage ---
above the pre-specified target of 85\% (Table~\ref{tab:simulation},
Figure~\ref{fig:ablation}B).
By contrast, the 95\% CI for $\alpha$ covered the true value in only 10\%
of seeds, substantially below target; we attribute this to the $F_3$ MOM
delta-method approximation underestimating uncertainty, the small simulation
$N_e = 1{,}000$ inflating genetic drift, and residual NJ-tree misplacement
errors (see Discussion).
...
$F_2$ denominators.
$T$ MAE was $10.7 \pm 4.0$ generations across both admixture events; notably,
the standard deviation of $T$ estimates was slightly lower in S3 than S2
(4.0 vs.\ 4.5 generations), suggesting that the increased number of
populations provides more calibrating IBD pairs.
The 95\% highest-density interval for $T$ (from MCMC) covered the true value
in 97.5\% of seeds --- above the pre-specified 85\% target and consistent
across both admixture events --- while the $\alpha$ delta-method CI coverage
of 20\% remained far below target for the same reasons as in S2
(see Discussion).
