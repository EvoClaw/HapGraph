...
Parameter metrics (columns 6--10) evaluate Stage~II under the oracle topology.
...
from the combined $F_2$+IBD likelihood (equation~\ref{eq:lik}).
...
$T$ CI width: mean width of the 95\% HDI for $T$ (generations);
wide CIs indicate low resolution, not precision.
...
\toprule
 & & \multicolumn{3}{c}{Topology (Stage I)} & \multicolumn{5}{c}{Parameters (Stage II, oracle topology)} \\
\cmidrule(lr){3-5}\cmidrule(lr){6-10}
Scenario & $K_{\mathrm{true}}$ & $K$-exact & Recall & Precision
         & $\alpha$ MAE & $T$ MAE (gen)
         & $\alpha$ CI cov. & $T$ CI cov. & $T$ CI width \\
\midrule
S1 (6 pops) & 0 & 1.00 & --- & --- & --- & --- & --- & --- & --- \\
